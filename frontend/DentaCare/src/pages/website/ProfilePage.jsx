...
export default function ProfilePage() {
  ...
  const verifyPaymentStatus = async (appointmentId, paymentMethod) => {
    try {
      ...
      const { data } = await api.post(endpoint, { appointmentId }, { headers: { token } });
  ...
  const getStatusBadge = (apt) => {
    ...
    if (apt.isPaid) return { text: t('confirmed'), color: 'bg-blue-100 text-blue-600', icon: CheckCircle };
                  ...
                  {editing ? (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <input type="email" value={profileData.email} disabled className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-gray-50 text-gray-500" />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input type="date" value={profileData.dob} onChange={e => setProfileData({ ...profileData, dob: e.target.value })} className="w-full mt-1 px-3 py-2 border border-border rounded-lg focus:outline-none focus:border-primary" />
                      </div>
                      <div>
                        <label className="text-sm font-medium text-text">{t('gender')}</label>
                        <select value={profileData.gender} onChange={e => setProfileData({ ...profileData, gender: e.target.value })} className="w-full mt-1 px-3 py-2 border border-border rounded-lg focus:outline-none focus:border-primary">
                      ...
                      <div>
                        ...
                        <textarea value={profileData.address} onChange={e => setProfileData({ ...profileData, address: e.target.value })} rows={2} className="w-full mt-1 px-3 py-2 border border-border rounded-lg focus:outline-none focus:border-primary" />
                      </div>
                    ...
                    <>
                      ...
                      <div className="mt-4 space-y-2 profile-info-item">
                        ...
                        {profileData.dob && <div className="flex items-center gap-2 text-sub"><Calendar size={14} /><span className="text-sm">{profileData.dob}</span></div>}
                        {profileData.gender && <div className="flex items-center gap-2 text-sub"><User size={14} /><span className="text-sm capitalize">{profileData.gender}</span></div>}
                        {profileData.address && <div className="flex items-start gap-2 text-sub"><MapPin size={14} className="mt-0.5" /><span className="text-sm">{profileData.address}</span></div>}
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scale-up">
            ...
            <div className="space-y-3">
              <button onClick={handleStripePayment} disabled={processingPayment === paymentAppointment._id} className="w-full flex items-center justify-between p-4 rounded-xl border-2 border-border hover:border-primary transition-all hover:shadow-md disabled:opacity-50">
                ...
                {processingPayment === paymentAppointment._id ? <div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin" /> : <ChevronRight size={20} className="text-muted" />}
              </button>
              <button onClick={handleChargilyPayment} disabled={processingPayment === paymentAppointment._id} className="w-full flex items-center justify-between p-4 rounded-xl border-2 border-border hover:border-primary transition-all hover:shadow-md disabled:opacity-50">
