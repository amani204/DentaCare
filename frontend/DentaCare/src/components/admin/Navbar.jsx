...
import { Languages, ChevronRight, LayoutDashboard, Stethoscope, UserPlus, Calendar  } from 'lucide-react';
...
export default function Navbar() {
  ...
  const routeConfigs = {
    '/admin': { label: t('dashboard'), icon: LayoutDashboard },
    '/admin/doctors': { label: t('doctors'), icon: Stethoscope },
    '/admin/add-doctor': { label: t('addDoctor'), icon: UserPlus },
    '/admin/appointments': { label: t('appointments'), icon: Calendar },
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2.5 group cursor-pointer">
          <div className="relative">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-primary-soft to-accent-soft  flex items-center justify-center text-xs font-bold text-primary-deep shadow-2xs group-hover:shadow-xs transition-all duration-200">
              A
            </div>
            {/* Tiny live system pulse indicator */}
            <span className="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-green-500 ring-2 ring-white" />
          </div>
          
          <div className="hidden sm:flex flex-col text-left">
            <span className="text-xs font-semibold text-text group-hover:text-primary transition-colors leading-none mb-1">
              Admin
            </span>
